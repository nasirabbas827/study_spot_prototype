...
if (isset($_GET["courseID"])) {
    ...
    if (mysqli_stmt_num_rows($stmt) == 0) {
        // User is not enrolled, so enroll them in the course
        $enrollQuery = "INSERT INTO enroll_courses (UserID, CourseID) VALUES (?, ?)";
        $stmt = mysqli_prepare($conn, $enrollQuery);
        mysqli_stmt_bind_param($stmt, "ii", $user_id, $courseID);
        
        if (mysqli_stmt_execute($stmt)) {
            // Enrollment successful, redirect to the dashboard
